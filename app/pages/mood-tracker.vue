  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="px-6 mb-6">
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-4 text-center">Pilih Mood Utama</h3>
        <div class="grid grid-cols-3 gap-4 mb-6">
          <button 
            ...
            :class="selectedMood === mood.id ? 'bg-gradient-to-br from-purple-500 to-pink-500 text-white shadow-lg' : 'bg-gray-50 hover:bg-gray-100'"
          >
            <span class="text-4xl mb-2" :class="selectedMood === mood.id ? 'animate-bounce' : ''">{{ mood.emoji }}</span>
            <span class="text-sm font-medium">{{ mood.label }}</span>
          </button>
        </div>
        
        <!-- Mood Intensity -->
        <div v-if="selectedMood" class="space-y-4">
          <div>
            <label for="mood-intensity" class="block text-sm font-medium text-gray-700 mb-2">Intensitas ({{ moodIntensity }}/10)</label>
            <div class="relative">
              <input 
                id="mood-intensity"
    ...
    <!-- Additional Emotions -->
    <div v-if="selectedMood" class="px-6 mb-6">
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Emosi Tambahan</h3>
        <p class="text-sm text-gray-600 mb-4">Pilih emosi lain yang kamu rasakan (opsional)</p>
        <div class="flex flex-wrap gap-2">
          <button 
            ...
            :class="selectedAdditionalEmotions.includes(emotion.id) 
    ...
    <div v-if="selectedMood" class="px-6 mb-6">
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Apa yang Mempengaruhi?</h3>
        <div class="grid grid-cols-2 gap-3">
          <button 
            ...
            :class="selectedTriggers.includes(trigger.id) 
    ...
    <div v-if="selectedMood" class="px-6 mb-6">
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Catatan (Opsional)</h3>
        <textarea 
          v-model="moodNotes"
    ...
    <!-- Save Button -->
    <div v-if="selectedMood" class="px-6 pb-24">
      <button 
        @click="saveMoodEntry"
        :disabled="!selectedMood"
        class="w-full py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
      >
        <CheckCircleIcon size="20" class="inline mr-2" />
    ...
    <div v-if="showHistory" class="fixed inset-0 bg-black/50 flex items-end z-50">
      <div class="bg-white w-full max-h-[80vh] rounded-t-3xl p-6 animate-slide-up overflow-y-auto">
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-7 gap-2">
            ...
            >
              ...
              <div class="text-lg">{{ day.mood ? getMoodEmoji(day.mood) : '😊' }}</div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          <div class="bg-purple-50 rounded-xl p-4 text-center">
            <div class="text-2xl font-bold text-purple-600">{{ moodStats.streak }}</div>
            <div class="text-sm text-purple-700">Hari Tracking</div>
          </div>
          <div class="bg-pink-50 rounded-xl p-4 text-center">
            <div class="text-2xl font-bold text-pink-600">{{ moodStats.averageIntensity }}</div>
        ...
        <!-- Recent Entries -->
        <div>
          <h4 class="font-semibold text-gray-900 mb-3">Entri Terbaru</h4>
          <div class="space-y-3">
            <div 
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="font-medium text-gray-900">{{ getMoodLabel(entry.mood) }}</div>
                    <div class="text-sm text-gray-600">Intensitas: {{ entry.intensity }}/10</div>
                  </div>
                </div>
                <div class="text-sm text-gray-500">{{ entry.date }}</div>
              </div>
              <p v-if="entry.notes" class="text-sm text-gray-700">{{ entry.notes }}</p>
            </div>
          </div>
        </div>
      </div>
...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Pantau dan catat suasana hati harianmu. Pahami pola emosi dan faktor yang mempengaruhi mood.' }
  ]
})
